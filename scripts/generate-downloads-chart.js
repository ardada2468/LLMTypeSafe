...
const MONTHS = 12;
const OUTPUT = path.join(__dirname, '..', 'assets', 'npm-downloads.svg');
...
async function monthlyTotals() {
  ...
  const from = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth() - RANGE_LIMIT_MONTHS, 1));
...
function renderSvg(data) {
  ...
  const peakIndex = data.reduce((best, d, i) => (d.downloads > data[best].downloads ? i : best), 0);
  ...
  const bars = data
    .map((d, i) => {
      ...
      const bar = barHeight
        ...
        : '';
      const value = labelled.has(i) && d.downloads > 0
...
async function main() {
  ...
  console.log(`Wrote ${path.relative(process.cwd(), OUTPUT)} (${data.length} months, ${data.reduce((s, d) => s + d.downloads, 0)} downloads)`);
}
